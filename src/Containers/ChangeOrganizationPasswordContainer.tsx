...
import { selectSignInEmail, selectSelectedOrg, setAuthData, selectTempOrg, setSelectedOrg, selectAuthData } from '@/Store/Auth'
...
const ChangeOrganizationPasswordContainer = ({ navigation }: Props) => {
  ...
  useEffect(() => {
    if (isSuccess && data && data.success) {
      ...
      console.log(`[ChangeOrganizationPasswordContainer] auth data: ${JSON.stringify(data.data)}`)
    } else if (isSuccess && !data?.success) {
      setError(true)
      console.log(`[ChangeOrganizationPasswordContainer] auth error: ${data?.error}`)
    }
